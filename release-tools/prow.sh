...
#
configvar CSI_PROW_E2E_FOCUS_LATEST '\[Feature:VolumeNfsExportDataSource\]' "non-alpha, feature-tagged tests for latest Kubernetes version"
...
# Which external-nfsexporter tag to use for the nfsexporter CRD and nfsexport-controller deployment
default_csi_nfsexporter_version () {
...
configvar CSI_NFSEXPORTTER_VERSION "$(default_csi_nfsexporter_version)" "external-nfsexporter version tag"
...
# Installs all necessary nfsexporter CRDs
install_nfsexport_crds() {
  # Wait until volumenfsexport CRDs are in place.
  CRD_BASE_DIR="https://raw.githubusercontent.com/kubernetes-csi/external-nfsexporter/${CSI_NFSEXPORTTER_VERSION}/client/config/crd"
  if [[ ${REPO_DIR} == *"external-nfsexporter"* ]]; then
      CRD_BASE_DIR="${REPO_DIR}/client/config/crd"
  fi
  echo "Installing nfsexport CRDs from ${CRD_BASE_DIR}"
  kubectl apply -f "${CRD_BASE_DIR}/nfsexport.storage.k8s.io_volumenfsexportclasses.yaml" --validate=false
  kubectl apply -f "${CRD_BASE_DIR}/nfsexport.storage.k8s.io_volumenfsexports.yaml" --validate=false
  kubectl apply -f "${CRD_BASE_DIR}/nfsexport.storage.k8s.io_volumenfsexportcontents.yaml" --validate=false
  cnt=0
  until kubectl get volumenfsexportclasses.nfsexport.storage.k8s.io \
    && kubectl get volumenfsexports.nfsexport.storage.k8s.io \
    && kubectl get volumenfsexportcontents.nfsexport.storage.k8s.io; do
    if [ $cnt -gt 30 ]; then
        echo >&2 "ERROR: nfsexport CRDs not ready after over 1 min"
        exit 1
    fi
    echo "$(date +%H:%M:%S)" "waiting for nfsexport CRDs, attempt #$cnt"
	cnt=$((cnt + 1))
    sleep 2
  done
}

# Install nfsexport controller and associated RBAC, retrying until the pod is running.
install_nfsexport_controller() {
  CONTROLLER_DIR="https://raw.githubusercontent.com/kubernetes-csi/external-nfsexporter/${CSI_NFSEXPORTTER_VERSION}"
  if [[ ${REPO_DIR} == *"external-nfsexporter"* ]]; then
      CONTROLLER_DIR="${REPO_DIR}"
  fi
  NFSEXPORT_RBAC_YAML="${CONTROLLER_DIR}/deploy/kubernetes/nfsexport-controller/rbac-nfsexport-controller.yaml"
  echo "kubectl apply -f ${NFSEXPORT_RBAC_YAML}"
  # Ignore: Double quote to prevent globbing and word splitting.
  # shellcheck disable=SC2086
  kubectl apply -f ${NFSEXPORT_RBAC_YAML}

  cnt=0
  until kubectl get clusterrolebinding nfsexport-controller-role; do
     if [ $cnt -gt 30 ]; then
        echo "Cluster role bindings:"
        kubectl describe clusterrolebinding
        echo >&2 "ERROR: nfsexport controller RBAC not ready after over 5 min"
        exit 1
    fi
    echo "$(date +%H:%M:%S)" "waiting for nfsexport RBAC setup complete, attempt #$cnt"
	cnt=$((cnt + 1))
    sleep 10
  done

  NFSEXPORT_CONTROLLER_YAML="${CONTROLLER_DIR}/deploy/kubernetes/nfsexport-controller/setup-nfsexport-controller.yaml"
  if [[ ${REPO_DIR} == *"external-nfsexporter"* ]]; then
      # nfsexport-controller image built from the PR will get a "csiprow" tag.
      # Load it into the "kind" cluster so that we can deploy it.
      NEW_TAG="csiprow"
      NEW_IMG="nfsexport-controller:${NEW_TAG}"
      echo "kind load docker-image --name csi-prow ${NEW_IMG}"
      kind load docker-image --name csi-prow ${NEW_IMG} || die "could not load the nfsexport-controller:csiprow image into the kind cluster"

      # deploy nfsexport-controller
      echo "Deploying nfsexport-controller from ${NFSEXPORT_CONTROLLER_YAML} with $NEW_IMG."
      # Replace image in NFSEXPORT_CONTROLLER_YAML with nfsexport-controller:csiprow and deploy
      ...
      for i in $(ls ${NFSEXPORT_CONTROLLER_YAML} | sort); do
          echo "   $i"
          ...
          modified="$(cat "$i" | while IFS= read -r line; do
              ...
              if echo "$nocomments" | grep -q '^[[:space:]]*image:[[:space:]]*'; then
                  # Split 'image: registry.k8s.io/sig-storage/nfsexport-controller:v3.0.0'
                  # into image (nfsexport-controller:v3.0.0),
      ...
      echo "Deploying nfsexport-controller from ${NFSEXPORT_CONTROLLER_YAML} with canary images."
      yaml="$(kubectl apply --dry-run=client -o yaml -f "$NFSEXPORT_CONTROLLER_YAML")"
      # Ignore: See if you can use ${variable//search/replace} instead.
      # shellcheck disable=SC2001
      modified="$(echo "$yaml" | sed -e "s;image: .*/\([^/:]*\):.*;image: ${CSI_PROW_DRIVER_CANARY_REGISTRY}/\1:canary;")"
      diff <(echo "$yaml") <(echo "$modified")
      if ! echo "$modified" | kubectl apply -f -; then
          echo "modified version of $NFSEXPORT_CONTROLLER_YAML:"
          echo "$modified"
          exit 1
      fi
  else
      echo "kubectl apply -f $NFSEXPORT_CONTROLLER_YAML"
      kubectl apply -f "$NFSEXPORT_CONTROLLER_YAML"
  fi

  cnt=0
  expected_running_pods=$(kubectl apply --dry-run=client -o "jsonpath={.spec.replicas}" -f "$NFSEXPORT_CONTROLLER_YAML")
  expected_namespace=$(kubectl apply --dry-run=client -o "jsonpath={.metadata.namespace}" -f "$NFSEXPORT_CONTROLLER_YAML")
  while [ "$(kubectl get pods -n "$expected_namespace" -l app=nfsexport-controller | grep 'Running' -c)" -lt "$expected_running_pods" ]; do
    if [ $cnt -gt 30 ]; then
        echo "nfsexport-controller pod status:"
        kubectl describe pods -n "$expected_namespace" -l app=nfsexport-controller
        echo >&2 "ERROR: nfsexport controller not ready after over 5 min"
        exit 1
    fi
    echo "$(date +%H:%M:%S)" "waiting for nfsexport controller deployment to complete, attempt #$cnt"
	cnt=$((cnt + 1))
    sleep 10
  done
}
...
main () {
    ...
    if tests_need_kind; then
        ...
        if tests_need_non_alpha_cluster; then
            ...
            # Install necessary nfsexport CRDs and nfsexport controller
            install_nfsexport_crds
            install_nfsexport_controller
