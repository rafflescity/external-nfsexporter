...
The nfsexport validating webhook is an HTTP callback which responds to [admission requests](https://kubernetes.io/docs/reference/access-authn-authz/extensible-admission-controllers/). It is part of a larger [plan](https://github.com/kubernetes/enhancements/tree/master/keps/sig-storage/1900-volume-nfsexport-validation-webhook#proposal) to tighten validation for volume nfsexport objects. This webhook introduces the [ratcheting validation](https://github.com/kubernetes/enhancements/tree/master/keps/sig-storage/1900-volume-nfsexport-validation-webhook#backwards-compatibility) mechanism targeting the tighter validation. The cluster admin or Kubernetes distribution admin should install the webhook alongside the nfsexport controllers and CRDs.
...
1. Run the `create-cert.sh` script. Note using the default namespace will allow anyone with access to that namespace to read your secret. It is recommended to change the namespace in all the files and the commands given below.
    ...
    ./deploy/kubernetes/webhook-example/create-cert.sh --service nfsexport-validation-service --secret nfsexport-validation-secret --namespace default # Make sure to use a different namespace
    ```
...
Try to create an invalid nfsexport object, the nfsexport creation should fail.

```bash
kubectl create -f ./examples/kubernetes/invalid-nfsexport-v1.yaml
```
...
Please see the deployment [yaml](./webhook.yaml) for the arguments expected by the webhook server. The nfsexport validation webhook is served at the path `/volumenfsexport`.
