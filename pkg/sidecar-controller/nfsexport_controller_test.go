...
func TestShouldDelete(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			content:        newContent("test-content", "snap-uuid", "snapName", "desiredHandle", "default", "desiredHandle", "volHandle", crdv1.VolumeNfsExportContentDelete, nil, &defaultSize, false, nil),
		},
		{
			name:           "Content is not bound",
			expectedReturn: true,
			content:        newContent("test-content-not-bound", "", "", "nfsexportHandle", "", "", "", crdv1.VolumeNfsExportContentDelete, nil, &defaultSize, false, &timeNowMetav1),
		},
		{
			name:           "AnnVolumeNfsExportBeingDeleted annotation is set. ",
			expectedReturn: true,
			// DeletionTime means that annotation is set, and being bound means the other cases are skipped.
			content: newContent("test-content", "snap-uuid", "snapName", "desiredHandle", "default", "desiredHandle", "volHandle", crdv1.VolumeNfsExportContentDelete, nil, &defaultSize, false, &timeNowMetav1),
		},
		{
