...
var (
	...
	emptySize     int64
	deletePolicy  = crdv1.VolumeNfsExportContentDelete
	retainPolicy  = crdv1.VolumeNfsExportContentRetain
	timeNow       = time.Now()
...
var class6Parameters = map[string]string{
	utils.PrefixedNfsExportterSecretNameKey:          "secret",
	utils.PrefixedNfsExportterSecretNamespaceKey:     "default",
	utils.PrefixedNfsExportterListSecretNameKey:      "secret",
	utils.PrefixedNfsExportterListSecretNamespaceKey: "default",
}
...
func TestDeleteSync(t *testing.T) {
	tests := []controllerTest{
		...
		{
			name:             "1-2 - content non-nil DeletionTimestamp with retain policy will not delete nfsexport",
		...
		{
			name:             "1-3 - delete nfsexport error should result in an event, bound finalizer should remain",
		...
		{
			name:                "1-4 - fail to delete with a nfsexport class which has invalid secret parameter, bound finalizer should remain",
		...
		{
			name:                "1-5 - csi driver delete nfsexport returns error, bound finalizer should remain",
		...
		{
			// delete success(?) - content is deleted before deleteNfsExport starts
		...
		{
			name:              "1-7 - content will not be deleted if it is bound to a nfsexport correctly, nfsexport uid is not specified",
		...
		{
			name:              "1-8 - content with retain policy will not be deleted if it is bound to a non-exist nfsexport and also has a nfsexport uid specified",
			initialContents:   newContentArrayWithReadyToUse("content1-8", "sid1-8", "none-existed-nfsexport", "sid1-8", validSecretClass, "sid1-8", "", retainPolicy, nil, &defaultSize, &True, true),
			expectedContents:  newContentArrayWithReadyToUse("content1-8", "sid1-8", "none-existed-nfsexport", "sid1-8", validSecretClass, "sid1-8", "", retainPolicy, nil, &defaultSize, &True, true),
			expectedEvents:    noevents,
