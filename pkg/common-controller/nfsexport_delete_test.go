...
var class4Parameters = map[string]string{
	// utils.NfsExportterSecretNameKey:      "emptysecret",
	// utils.NfsExportterSecretNamespaceKey: "default",
}
	...
	tests := []controllerTest{
		...
		},
		{
			...
			expectedContents: withContentAnnotations(newContentArray("snapcontent-snapuid3-2", "snapuid3-2", "snap3-2", "sid3-2", validSecretClass, "", "volume3-2", deletePolicy, nil, nil, true),
				map[string]string{
					"nfsexport.storage.kubernetes.io/volumenfsexport-being-deleted": "yes",
				}),
			...
			errors: []reactorError{
				...
				{"delete", "volumenfsexportcontents", errors.New("mock delete error")},
			},
			expectSuccess: false,
			test:          testSyncNfsExportError,
		},
		{
			name:            "3-3 - (dynamic) content will not be deleted if retainPolicy is set, nfsexport should have its finalizer removed",
		...
		{
			name:            "3-8 - (static) content will not be deleted if deletion API call fails, nfsexport should have its finalizers remained",
		...
		{
			name:            "3-9 - (static) content will not be deleted if retainPolicy is set, nfsexport should have its finalizer removed",
