...
func TestGetSecretReference(t *testing.T) {
	testcases := map[string]struct {
		...
		"template - invalid": {
			secretParams: NfsExportterSecretParams,
			params: map[string]string{
				PrefixedNfsExportterSecretNameKey:      "static-${volumenfsexportcontent.name}-${volumenfsexport.namespace}-${volumenfsexport.name}-${volumenfsexport.annotations['akey']}",
				PrefixedNfsExportterSecretNamespaceKey: "static-${volumenfsexportcontent.name}-${volumenfsexport.namespace}",
			},
...
func TestRemovePrefixedCSIParams(t *testing.T) {
	testcases := []struct {
		...
		{
			name:           "one prefixed",
			params:         map[string]string{PrefixedNfsExportterSecretNameKey: "bar", "bim": "baz"},
			expectedParams: map[string]string{"bim": "baz"},
		},
		{
			name: "all known prefixed",
			params: map[string]string{
				PrefixedNfsExportterSecretNameKey:          "csiBar",
